...
class _ThresholdDistancePageState extends State<ThresholdDistancePage>
  ...
  Future<void> uploadThDistance() async {
    var response = await http.put(
        Uri.parse(globals.url + "/scripts/threshold-distance-script"),
        ...
        body: convert.jsonEncode(<String, String>{
          'code':
              "alert().filter(a<" + _thresholdDistance.toString() + ").send()",
        }));
    print(response.statusCode);
    print(response.body);
    if (response.statusCode == 200) {
      print("distanza soglia modificata con successo");
    }
  }
}
